var URLtoXML =
{
     ...
     fMode : true, //режим обмена данными (асинхронный=true синхронный=false)
	  ...
	  arrVideoExt : [".flac", ".ts", ".mp3", ".avi", ".asf", ".asx", ".3gp", ".3g2", ".3gp2", ".3gpp", ".flv", ".mp4", ".mp4v", ".m4v", ".m2v", ".m2ts", ".m2t", ".mp2v", ".mov", ".mpg", ".mpe", ".mpeg", ".mkv", ".swf", ".mts", ".wm", ".wmx", ".wmv", ".vob", ".iso", ".f4v"],
};
  ...
  var index3=0; //индекс масcива
   ...
   if (this.xmlHTTP.status == 200 )//если ответ от сервера корректный
   {
	...
	if(Main.playlist==0){ 
	  ...
	  //alert(Main.search);
	  //ищем ссылки страниц
	   sUrl = URLtoXML.FindVal(sOut, this.nStart, "<td><a href='/view/", Main.search); // для поиска "'>" иначе "?"
         //alert(sUrl);
	   if (sUrl != "" && sUrl.length >=5) {//
	    ...
	    if (sImg != "" && sImg.length>=25){
			...
			document.getElementById("bloc"+index).innerHTML = "<img id='imgst"+index+"';  src='" + sImg + "' />" ; //style='border-style: solid; border-width:10px; border-color:#e9e9e9;'
            }
	     //ищем описание
		 TMPnStart = this.nStart;
        ...
        if (sDes != "") {
	      ...
	      //Ищем заголовок в описании
           TMPnStart = this.nStart;
           sTit = URLtoXML.FindVal(sDes, 0, "alt='", "'>");
         
           this.nStart = TMPnStart;	//возвращаем значение глобального поиска        
			//Если нашли заголовок иссключаемa лишнее
            if (sTit != "" && sTit.indexOf('EX')== -1  && sTit.indexOf('перейти на первую страницу')== -1 && sTit.indexOf('перейти на предыдущую страницу') == -1 && sTit.indexOf('перейти на следующую страницу')== -1 && sTit.indexOf('перейти на последнюю страницу') == -1) {
	...
	if(this.sName[Main.index].length >=45){document.getElementById("title").innerHTML = this.sName[Main.index].substr(0,45)+"...";}//название в заголовок
	else{document.getElementById("title").innerHTML= this.sName[Main.index]};
	document.getElementById("imgst" + Main.index).style.borderColor="#fe761c"; //  активная строка
    ...
    for (var i in this.arrVideoExt){
      var wrd = this.arrVideoExt[i];//слово из массива
		   ...
		   if (sUrl.length !=0 && sUrl.indexOf(wrd)>=0){
			 ...
			 // выводи имена в список плейлисста (обрезаем имя после 37 символа и добавляемa многоточье )
			 if(pTit.length >=37 && index<100){document.getElementById("str" + index).innerHTML=pTit.substr(0,37)+"...";}
			 else if (index<100){document.getElementById("str" + index).innerHTML= pTit;}
...
var nEnd, sRes;
    ...
    if (nBeg >= 0){//если значение найдено
       ...
       nBeg = nBeg + keyBVal.length;//передвигаем начало поиска на след. символ за первичным ключом
       //ищем вторичный ключ
       nEnd = sRes.indexOf(keyEVal.toLowerCase(), nBeg);
       this.nStart = nEnd + keyEVal.length;//если не нашли окончание значения - становимся в конец строки +1 символ
