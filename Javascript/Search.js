...
Search.imeReady = function (imeObject) {
    ...
    document.getElementById("plainText").focus(); // помещаем фокус на вод текста
...
Search.onEnter = function (string) {
    //document.getElementById('notifications').innerHTML = rezylt; // готовый результат
	
	
    document.getElementById("anchor").focus(); //  перемещаем фокус на элемент "anchor"
...
Search.SearchOk = function (arg) { // отлавливаем текст и помещаем в переменную
    //document.getElementById('notifications').innerHTML = arg; // готовый результат
    rezylt = arg;
};